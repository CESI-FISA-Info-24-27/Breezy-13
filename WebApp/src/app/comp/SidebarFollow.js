...
export default function SideBarFollow({ style }) {
    ...
    async function fetchFollows() {
      try {
        ...
        if (!aborted && Array.isArray(data)) {
          console.log("Follows récupérés :", data);
          setFollows(data);
        }
      } catch (err) {
        console.error("Erreur getFollows :", err);
      }
    }
          ...
          <>
            {follows.slice(0, showCount).map((follow) => (
              ...
              >
                ...
                <span className="truncate">{follow.username || "Inconnu"}</span>
              </li>
            ))}
